...
function msaStatsSetup(target) {
 	// create stats
 	msaStats = new Stats;
	msaStats.domElement.style.position = 'absolute';
	msaStats.domElement.style.top = '0px';
	msaStats.domElement.style.zIndex = 100;
	target.appendChild( msaStats.domElement );
}

function msaStatsUpdate() {
	msaStats.update();
}